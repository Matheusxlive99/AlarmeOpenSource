...
public class AlarmViewHolder extends RecyclerView.ViewHolder {
    ...
    public void bind(Alarm alarm) {
        ...
        if (alarm.isRecurring()) {
            ...
            alarmRecurringDays.setText("Uma vez");
        }

        if (alarm.getTitle().length() != 0) {
            alarmTitle.setText(String.format("Título: %s", alarm.getTitle(), alarm.getAlarmId(), alarm.getCreated()));
        } else {
            alarmTitle.setText(String.format("%s", "Sem Título", alarm.getAlarmId(), alarm.getCreated()));
        }
